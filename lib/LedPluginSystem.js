...
class LedPluginSystem
{
  ...
  loadState() 
  {
    let state = path_modle.join(global.appRoot, 'state', 'plugin_state.json');
    if(fs.existsSync(state)) 
    {
      let loadState = require(state);
      if(!('pluginStates' in loadState))
        return false;  
      
      for (let plugin in this.module_holder)
      {
        if (!this.module_holder.hasOwnProperty(plugin)) { continue; }
        if (!(plugin in loadState.pluginStates)) { continue; }

        let module = this.module_holder[plugin];
        if('setStatus' in module)
        {
          module.setStatus(loadState.pluginStates[plugin]);
        }
      }
  ...
  saveState() 
  {
    ...
    try
    {
      ...
      for (let plugin in this.module_holder)
      {
        ...
        let module = this.module_holder[plugin];
        if('getStatus' in module)
        {
          pluginStates[plugin] = module.getStatus();
        }
      }
